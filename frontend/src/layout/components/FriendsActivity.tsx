...
import { HeadphonesIcon, Music, Scroll, Users } from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { useEffect } from "react";


const FriendsActivity = () => {
  const { users,isLoading,error,fetchUser } = useChatStore();
  const {user} = useUser();
  useEffect(() => {
    if(user){
      fetchUser();
    ...
    <div className="h-full bg-zinc-900 flex flex-col rounded-lg ">
      <div className="p-4 flex justify-between items-center border-b border-zinc-800">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">what they are listening to</h2>
            ...
            <div key={user._id} className="cursor-pointer hover:bg-zinc-800/50 p-3 rounded-md transition-colors group">
              <div className="flex items-start gap-3">
                <div className="relative">
                    <Avatar className="size-10 broder border-zinc-800">
